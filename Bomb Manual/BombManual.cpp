...
BombManual::BombManual() { //. = dot; - = dash
...
void BombManual::complicated()
{
	short numberWire, batt = -1;
	char serialEven = '\0', parallel = '\0';
	cout << "How many wires are there? ";
	cin >> numberWire; //gets the number of wires
	cin.get(); //trashes the character at the end of an input
	for (short i = 0; i < numberWire; i++) {
		string wireInfo;
		bool red = false, blue = false, led = false, star = false, needSerialEven = false, needParallel = false, needBatt = false;
		cout << "What does the wire have? Write it as one word: ((r)ed, (b)lue, (L)ED, (s)tar) ";
		getline(cin,wireInfo);
		transform(wireInfo.begin(),wireInfo.end(),wireInfo.begin(),::tolower);
		if (wireInfo.find("r") != string::npos)
			red = true;
		if (wireInfo.find("b") != string::npos)
			blue = true;
		if (wireInfo.find("l") != string::npos)
			led = true;
		if (wireInfo.find("s") != string::npos)
			star = true;
		if (red && blue && led && star)
			cout << "Don't cut it\n";
		else if (red && blue && led)
			needSerialEven = true;
		else if (red && blue && star)
			needParallel = true;
		else if (blue && led && star)
			needParallel = true;
		else if (blue && led)
			needParallel = true;
		else if (blue && star)
			cout << "Don't cut it\n";
		else if (red && led && star)
			needBatt = true;
		else if (red && led)
			needBatt = true;
		else if (red && star)
		...
		if (needParallel) {
			if (parallel == '\0') {
				cout << "Does the bomb have a parallel port? (y/n) ";
				cin >> parallel;
				cin.get();
			}
			if (parallel == 'y')
		...
		if (needBatt) {
			if (batt == -1) {
				cout << "How many batteries are on the bomb? ";
				cin >> batt;
				cin.get();
			}
			if (batt > 1)
